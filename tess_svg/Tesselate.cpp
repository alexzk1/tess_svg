...
const Vertexes &Tesselate::process(const Loops &vertexes, bool contourOnly)
{
    ...
    tess.reset(gluNewTess(), [](auto ptr) {
        if (ptr != nullptr)
        {
            gluDeleteTess(ptr);
        }
    ...
    std::deque<std::array<GLdouble, 3>> raw_coords_pool;
    auto l_combine = to_glu_callback<GLU_TESS_COMBINE>(
      [&](GLdouble *coords, GLdouble **, GLfloat *, GLdouble **dataOut) {
          raw_coords_pool.push_back({coords[0], coords[1], coords[2]});
          *dataOut = raw_coords_pool.back().data();
          spareverts.emplace_back(*coords, *(coords + 1));
      });
