...
const SvgPath::transform_funcs SvgPath::transforms = {
  ...
  SvgPath::func_holder("matrix(",
                       [](auto &m, auto &v2) {
                           used_vectors_type vals[] = {std::stod(*v2++), std::stod(*v2++),
                                                       std::stod(*v2++), std::stod(*v2++),
                                                       std::stod(*v2++), std::stod(*v2++)};
                           for (auto j = 0; j < 3; j++)
                           {
                               m(0, j) = vals[j * 2 + 0];
...
void SvgPath::parse_node(const pugi::xml_node &node)
{
    ...
    auto trans = std::string(parentNode.attribute("transform").as_string("translate(0,0)"));
    // std::tolower(std::string(parentNode.attribute("transform").as_string("translate(0,0)")),std::locale::classic());
...
bool SvgPath::func_holder::exec(const std::string &src, GlVertex::trans_matrix_t &res) const
{
    ...
    if (pos != std::string::npos)
    {
        std::regex rgx("\\s+|\\,+");
        pos += mask.length();
        std::sregex_token_iterator iter(src.begin() + pos, src.end(), rgx, -1);
        func(res, iter);
        return true;
    }
    return false;
