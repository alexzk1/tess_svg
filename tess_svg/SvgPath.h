...
private:
    ...
    private:
        typedef std::function<void(GlVertex::trans_matrix_t&, std::sregex_token_iterator&)> lambda_t;
        const std::string mask;
        const lambda_t func;
    public:

        func_holder(const std::string &mask, const lambda_t& func) : mask(mask), func(func)
        { }
